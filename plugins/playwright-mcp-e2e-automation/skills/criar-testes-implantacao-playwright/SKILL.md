...
description: "Criar e validar jornadas smoke Playwright quando o pedido declarar `MODO: Implantacao`, com URL, AGENTS.md, raiz do codigo-fonte e um caminho individual ou blocos numerados `CASO DE USO n`. Analise as fontes ligadas aos casos e aos produtores estritamente necessarios de suas precondicoes. Agrupe casos dependentes por entidade e runId, inclusive com perfis diferentes, preserve formularios multipagina e trate autocompletes sem nomes hardcoded. Nao usar com guia, testes negativos de formato ou `MODO: Geracao de massa de dados`."
---

# Criar Smoke Tests De Implantacao Playwright

Gerar smoke tests portateis a partir de um caminho individual ou de casos explicitos. Casos dependentes formam uma jornada transacional quando compartilham a mesma entidade e o mesmo `runId`, ainda que exijam perfis autenticados diferentes.
...
- numerado: um ou mais blocos `CASO DE USO <n>` com `OPERACAO`, `CAMINHO`, `PERFIL`, `USUARIO`, `SENHA` e `RESULTADO ESPERADO` no proprio bloco. `MASSA E PRE-CONDICOES`, `DADOS ESPECIFICOS` e `OBSERVACOES` sao opcionais; quando a massa nao for informada, o preflight deve procurar seu produtor seguro pela interface.
...
Gravar credenciais reais somente em `.env`, manter `.env` ignorado e criar `.env.example` sem segredos. Normalizar o perfil para as chaves `E2E_<PERFIL>_USERNAME` e `E2E_<PERFIL>_PASSWORD`. Em implantacao, manter em `tests/auth/specProfiles.js` um mapeamento simples `{ id, perfil, arquivo }` para cada combinacao de perfil e spec; o mesmo arquivo pode aparecer varias vezes com `id` e perfil distintos. Fazer o `globalSetup` autenticar sequencialmente, com trace desabilitado, criando um `storageState` temporario por combinacao de perfil e spec em `test-results/auth/`. A pagina principal usa seu estado e cada papel adicional usa `criarPaginaAutenticada(perfil, specId)`, que abre um novo contexto com `obterCaminhoEstadoAutenticacao(perfil, specId)`; nunca compartilhar contexto ou `JSESSIONID` entre papeis. A spec inicia confirmando cada sessao autenticada, sem login ou preenchimento de credenciais dentro do teste reportado. O `globalSetup` deve ler os filtros de arquivo do comando em `config.argv`: sem filtro, preparar todas as specs; com um ou mais filtros, preparar somente as entradas correspondentes, inclusive todos os perfis mapeados para o arquivo selecionado. Filtrar antes de ler credenciais ou autenticar, falhar quando um filtro de arquivo informado nao corresponder ao mapeamento e nao exigir variavel adicional para escolher specs. No UI Mode, o Playwright chama o `globalSetup` antes de exibir a lista; ao detectar `--ui`, retornar antes de abrir browser ou ler credenciais. Adicionar uma fixture automatica que, somente depois de o usuario executar uma spec na interface, identifica todas as entradas por `testInfo.file`, autentica cada perfil em processo/contexto separado sem trace e grava os estados consumidos pela jornada. Casos com mesmo perfil e credenciais reutilizam as variaveis, mas nao um arquivo de estado entre specs. Se o mesmo perfil aparecer com credenciais diferentes, bloquear os casos envolvidos ate a inconsistencia ser resolvida. Nunca copiar credenciais para spec, Page Object, cache, log, trace, anexo ou relatorio. Nao exigir `E2E_CLIENT_PROFILE`, `config/defaults.json`, `config/clientes` ou preflight de massa por cliente.

## Processamento Dos Casos

- Seguir o `CAMINHO` de cada caso e somente os caminhos produtores estritamente necessarios para criar suas precondicoes pela interface; nao explorar funcionalidades vizinhas sem relacao produtora nem transformar botoes tecnicos em novos casos.
...
- Manter uma unica sessao MCP por perfil em cada fluxo multipagina e preservar a correlacao pelo mesmo `runId` ao alternar papeis. Nao abrir probes independentes, repetir login do mesmo perfil ou iniciar o formulario novamente para descobrir a tela seguinte.
...
Casos de alteracao, remocao ou transicao irreversivel nao podem atuar sobre massa preexistente. A jornada deve primeiro tentar criar e localizar o alvo atual com `runId`, inclusive por etapa de suporte e outro perfil. Bloquear somente quando as fontes e a interface comprovarem que nenhum produtor seguro esta disponivel, informando o produtor ou perfil ausente e a alternativa concreta.
...
1. Criar um unico `test` quando as operacoes compartilham a mesma entidade, o mesmo `runId` e o mesmo ciclo transacional, inclusive quando a jornada alternar perfis por contextos autenticados separados. Operacoes realmente independentes podem usar testes separados, cada um com pagina, autenticacao e massa proprios.
...
- Executar `check-environment.mjs <raiz> --headed-smoke --json`, `optimize-context.mjs` e, se necessario, `scaffold-playwright.mjs <raiz> --mode implantacao`. O scaffold deve permanecer minimo em todos os modos: configuracao Playwright, fixture maximizada, perfis de autenticacao e dados de teste. Implementar o `globalSetup` seguro somente junto das specs que realmente o consumirem e permitir varias entradas de perfil para o mesmo arquivo. Essas ferramentas nao devem criar massa funcional, cache nem infraestrutura de relatorio no projeto.
...
- Classificar a jornada pelos casos mais restritivos entre `formulario`, `consulta`, `relatorio`, `remocao` ou `transicao` e executar `quality-gate.mjs <raiz> --contract implantacao --case-kind <tipo> --expected-step <operacao> --files <specs> <pages>`, repetindo `--expected-step` para todos os casos solicitados. O gate coleta as specs selecionadas com `playwright test --list`, aceita varios perfis para o mesmo arquivo e reprova caso sem cobertura, dependencia entre specs, setup funcional oculto ou mutacao sem alvo proprio. Depois executar uma unica execucao headed dos arquivos aprovados, passando os proprios arquivos no comando Playwright e mantendo `workers=1`, para produzir o HTML nativo sem duplicar registros. O mesmo comando sem filtro de arquivo executa a suite completa.
...
Entregar uma spec simples por caso isolado ou jornada compativel, Page Objects reutilizados, utilitarios realmente usados, `.env.example`, comandos headed padrao do Playwright para uma spec e para a suite, mapeamento completo dos casos para specs/etapas, perfis usados, produtores de precondicao, caminho do HTML nativo e bloqueios reais. Informar tentativas e residuos apenas na resposta, sem criar infraestrutura de recuperacao no projeto.
