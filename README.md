...
Esse modo sempre usa `AGENTS.md` e código-fonte como evidência, lendo as partes ligadas aos casos e aos produtores estritamente necessários de suas precondições. Antes de bloquear por falta de massa, o plugin tenta reutilizar saída da jornada, criar um alvo sintético pela interface, qualificar de modo reversível um candidato institucional quando a elegibilidade só aparece após uma ação e alternar para outro perfil. Massa preexistente fica restrita a consultas e operações comprovadamente não destrutivas. Um módulo adicional não é presumido como fonte auxiliar: ele precisa estar nos casos ou ser um produtor obrigatório comprovado e disponível. Jornadas legitimamente multimódulo continuam aceitas. Cada spec representa uma operação isolada ou uma jornada compatível, com uma massa principal, contextos autenticados separados por papel, cobertura dos botões seguros, checkpoints de erro impeditivo e conclusões funcionais visíveis.
...
Listas alimentadas por cadastros anteriores usam a primeira opção válida, ignorando placeholders, valores vazios, opções ocultas e desabilitadas. Em autocomplete, um valor informado é pesquisado diretamente e exige correspondência exata; sem valor informado, aplicações compatíveis podem usar `%%%` para listar candidatos. Quando a elegibilidade não estiver exposta na lista, o plugin só faz qualificação sistemática se existir uma ação anterior à persistência principal que aplique a mesma regra, possa ser integralmente desfeita e permita comprovar o retorno ao estado inicial. A cardinalidade vem dos papéis consumidores da jornada, não de um número global. Nomes presumidos e tentativas aleatórias não entram no código. Valores de domínio que alteram o significado do registro continuam explícitos.
